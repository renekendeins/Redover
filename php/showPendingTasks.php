...
$query = "SELECT ID, PRIORITY, NAME, DESCRIPTION, END, START, STATUS, DATE, CREATOR, DOER FROM list_n  WHERE STATUS = 1 OR STATUS = 3 OR STATUS = 4 ORDER BY PRIORITY ASC";
    ...
    while($row = $result -> fetch(PDO::FETCH_ASSOC)){
		...
		echo '<td><div id="info_A'.$counter.'" onclick="toggleInfo(this.id)" class="pointer hover-bc-yellow" style="line-height:1.3; width:30px;height:30px;background-color:rgb(43,178,221);font-size:24px;text-align:center;position:relative;"><i class="fa fa-info"></i>
				<ul id="infoPanel_A'.$counter.'" style="width:300px; font-size:15px; list-style:none;padding-left:0px;background-color:#d9ddde; border:1px solid rgb(180,180,180);padding:5px; display:none; position:absolute;top:25px;z-index:1" class="txt-left" >
	...
	echo '	<td style="text-align: right;"><button type="submit" name="delete"  class="delete">DELETE</button>';
